...
<form method="post" action="<?= $product ? site_url('products/'.$product['product_id']) : site_url('products') ?>">
    ...
    <div style="display:flex;gap:8px"><input id="product_name" name="product_name" required maxlength="160" value="<?= esc(old('product_name', $product['product_name'] ?? '')) ?>"><button class="btn secondary" type="button" id="scan-fruit">Scan camera</button></div>
...
scanButton.addEventListener('click', async () => {
    try {
        ...
        if (!result.top) throw new Error('No fruit detected');
